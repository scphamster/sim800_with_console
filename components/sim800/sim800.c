...
parse_object(cJSON *item)
{
    ...
    while (subitem) {
        ...
        if (subitem->type == cJSON_String)
            ...
            printf("True\r\n");
        else if (subitem->type == cJSON_NULL)
            printf("NULL\r\n");
            ...
            for (n = 0; n < 3; n++) {
                ...
                if ((arritem) && (arritem->valuestring))
                    printf("   %s\n", arritem->valuestring);
                else
                    break;
            }
...
//-------------------------------------------------------------------
esp_err_t
esp32_wifi_eventHandler(void *ctx, system_event_t *event)
{
...
//----------------------------------------------------------------------------------------
static void
http_get_fromInternet(struct netconn *conn, const char *request, uint8_t type)
{
...
sim800_startUP(void)
{
    ...
    if (ppposInit() == 0) {
        ESP_LOGE("PPPoS EXAMPLE", "ERROR: GSM not initialized, Deleting GSM Task");
    ...
    // if (err != ESP_OK) {
    //     esp_err_to_name_r(err, err_buf, CONFIG_ERR_BUFLEN);

    //     ESP_LOGE(
    //       TAG,
    //       "Error ocured during obtainment of current time \nError code is %d \nError is:%s",
    //       err,
    //       err_buf);
    // }
