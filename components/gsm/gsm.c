...
static int g_uart_module_num = UART_NUM_1;
...
//---------------------------------------------------------
static void post_command_to_ESP_LOGI(char *cmd, int cmdSize, char *TAG2)
{
	char buf[cmdSize + 2];
...
//----------------------------------------------------------------------------------------------------------------------
static int sendATcmd_checkAnswer(char *cmd, char *desired_resp_pri, char *desired_resp_sec, int cmdSize, int timeout, char **resp_return_buff, int size)
{
	char resp_buf[256] = {'\0'};
	char buf[256] = {'\0'};
    int uart_data_len;
	...
	if (resp_return_buff != NULL) {
		...
		uart_data_len = uart_read_bytes(g_uart_module_num, (uint8_t*)buf, 256, timeout / portTICK_RATE_MS);
		...
		while (uart_data_len > 0) {
			...
			uart_data_len = uart_read_bytes(g_uart_module_num, (uint8_t*)buf, 256, 100 / portTICK_RATE_MS);
		}
	...
	while(1)
	{
		...
		uart_data_len = uart_read_bytes(g_uart_module_num, (uint8_t*)buf, 256, 10 / portTICK_RATE_MS);
...
//----------------------------
static void _reset_init_commands()
{
...
static void pppos_client_task()
{
	...
	if (uart_param_config(g_uart_module_num, &uart_config)) goto exit;
	//Set UART1 pins(TX, RX, RTS, CTS)
	if (uart_set_pin(g_uart_module_num, UART_GPIO_TX, UART_GPIO_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE)) goto exit;
	if (uart_driver_install(g_uart_module_num, BUF_SIZE * 2, BUF_SIZE * 2, 0, NULL, 0)) goto exit;
	...
	while(1)
	{
		...
		while(gsmCmdIter < GSM_InitCmdsSize)
		{
			if (GSM_Init[gsmCmdIter]->skip) {
				#if GSM_DEBUG
				post_command_to_ESP_LOGI(GSM_Init[gsmCmdIter]->cmd, GSM_Init[gsmCmdIter]->cmdSize, "Skip command:");
				#endif
				gsmCmdIter++;
				continue;
			}

			if (sendATcmd_checkAnswer(GSM_Init[gsmCmdIter]->cmd,
		...
		if (g_gsm_status == GSM_STATE_FIRSTINIT) {
			...
			#if GSM_DEBUG
			ESP_LOGI(TAG, "pppapi_pppos_create\n file = %s, line = %d", __FILE__, __LINE__);
			#endif
		...
		while(1) {
			...
			//if disconnection requested???
			if (g_do_pppos_connect <= 0) {
				int end_task = g_do_pppos_connect;
				...
				while (g_gsm_status != GSM_STATE_DISCONNECTED) {
					// Handle data received from GSM
					int len;
					
					memset(data, 0, BUF_SIZE);
					len = uart_read_bytes(g_uart_module_num, (uint8_t*)data, BUF_SIZE, 30 / portTICK_RATE_MS);
				...
				//clear and reset progress of initialisation
				gsmCmdIter = 0;
				_reset_init_commands();
			...
			int len = uart_read_bytes(g_uart_module_num, (uint8_t*)data, BUF_SIZE, 30 / portTICK_RATE_MS);
...
void ppposDisconnect(uint8_t end_task, uint8_t rfoff)
{
	...
	if (gsm_status == GSM_STATE_IDLE) return;
...
//--------------------
static int module_is_in_text_mode_and_active()
{
